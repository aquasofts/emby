...
download_file(){
# nginx配置文件下载地址
url="https://raw.githubusercontent.com/aquasofts/emby/main/file/Rewrite/autohttps"

# 目标文件路径（包括文件名）
destination="/etc/nginx/sites-available/autohttps"
...
Replace(){
# 替换/etc/nginx/sites-available/autohttps中的yourdomain为用户输入的域名
sudo sed -i "s|yourdomain|$domain|g" /etc/nginx/sites-available/autohttps
if [ $? -ne 0 ]; then
...
# 替换/etc/nginx/sites-available/autohttps中的embydomain为用户输入的emby域名
sudo sed -i "s|embydomain|$content2|g" /etc/nginx/sites-available/autohttps
if [ $? -ne 0 ]; then
    echo "反代的域名替换失败，请检查文件路径和权限。" >&2
    exit 1
fi

# 替换/etc/nginx/sites-available/autohttps中的jjkk为证书公钥目录
sudo sed -i "s|jjkk|$domain|g" /etc/nginx/sites-available/autohttps
if [ $? -ne 0 ]; then
    echo "证书公钥替换失败，请检查文件路径和权限。" >&2
    exit 1
fi

# 替换/etc/nginx/sites-available/autohttps中的hhjj为证书公钥目录
sudo sed -i "s|hhjj|$domain|g" /etc/nginx/sites-available/autohttps
if [ $? -ne 0 ]; then
...
Link(){
# 链接配置
if [ -f /etc/nginx/sites-available/autohttps ]; then
    sudo ln -sf /etc/nginx/sites-available/autohttps /etc/nginx/sites-enabled/
    if [ $? -ne 0 ]; then
        echo "配置链接失败，请检查文件路径和权限。" >&2
        exit 1
    fi
else
    echo "/etc/nginx/sites-available/autohttps 文件不存在。" >&2
    exit 1
fi
}
