...
# nginx配置文件下载地址
url="https://raw.githubusercontent.com/aquasofts/emby/main/file/Rewrite/http"

# 目标文件路径（包括文件名）
destination="/etc/nginx/sites-available/http"
...
# 替换/etc/nginx/sites-available/http中的yourdomain为用户输入的域名
sudo sed -i "s|yourdomain|$domain|g" /etc/nginx/sites-available/http
if [ $? -ne 0 ]; then
...
# 替换/etc/nginx/sites-available/http中的embydomain为用户输入的emby域名
sudo sed -i "s|embydomain|$content2|g" /etc/nginx/sites-available/http
if [ $? -ne 0 ]; then
...
if [ -f /etc/nginx/sites-available/http ]; then
    sudo ln -sf /etc/nginx/sites-available/http /etc/nginx/sites-enabled/
    if [ $? -ne 0 ]; then
        echo "配置链接失败，请检查文件路径和权限。" >&2
        exit 1
    fi
else
    echo "/etc/nginx/sites-available/http 文件不存在。" >&2
    exit 1
fi
